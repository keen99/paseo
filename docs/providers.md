...
### Pi live-bridge command-output persistence

The live Pi bridge captures `ctx.ui.notify()` while executing a browser-submitted extension command and publishes the notification as an assistant timeline item. This includes output from commands such as `/usage`.

These notification items are **live-only**. Pi does not write UI notifications to its session JSONL. After the Paseo daemon restarts, timeline hydration uses Pi's persisted JSONL, so previously relayed command notifications disappear from the browser, even when the browser page itself was not reloaded. Normal Pi user and assistant messages remain available because Pi persists them.

Do not describe live command notifications as durable transcript history. Making them durable requires either a separate Paseo timeline store that survives daemon restart or explicit persistence into Pi's session as a custom message.
